...
def shear_x_func(img, factor, fill=(0, 0, 0)):
    ...
    out = cv2.warpAffine(img, M, (W, H), borderMode=cv2.BORDER_REPLICATE, flags=cv2.INTER_LINEAR).astype(np.uint8)
    return out
...
def enhance_level_to_args(MAX_LEVEL):
    def level_to_args(level):
        return ((level / MAX_LEVEL) * 1.4 + 0.1,)
    return level_to_args


def shear_level_to_args(MAX_LEVEL, replace_value):
    def level_to_args(level):
        level = (level / MAX_LEVEL) * 0.25
